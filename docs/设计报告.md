...
```mermaid
flowchart LR
    CoordClient[Coordinator Client CLI] -->|HTTP POST /sql 等| Coordinator[Coordinator]
    TestScript[自动化测试脚本] -->|HTTP| Coordinator
    NodeClient[DataNode Local CLI] -->|进程内 executeLocalSql| DataNode[DataNode]
...
`DisMiniSql` 是统一命令行入口，支持五种角色：

- `init-zk <config>`：初始化 ZooKeeper 中的分片元数据。
- `coordinator <config>`：启动后台 Coordinator HTTP 服务。
- `client <config>`：启动 **Coordinator Client**（`CoordinatorCli`），通过 HTTP 连接已运行的 Coordinator。
- `datanode <config> <nodeId>`：启动指定 DataNode HTTP 服务。
- `datanode <config> <nodeId> --cli`：启动 DataNode 后进入 **DataNode Local CLI**（`DataNodeCli`），在本节点进程内直接执行 SQL，不经过 Coordinator。
...
`coordinator` 模块是查询入口和分布式执行核心。`CoordinatorServer` 暴露 HTTP 接口并实现 SQL 分类、写入路由、读请求分发、admin 运维接口，同时提供可复用的 `nodes`、`metadata`、`health`、`rebalance`、`snapshot` 方法供 HTTP handler 使用。`CoordinatorCli` 是面向集群用户的 **Coordinator Client**：与 Coordinator 进程分离，只通过 HTTP 访问后台服务；用户可直接输入 SQL，也可执行 `shard`、`nodes`、`metadata`、`health`、`rebalance`、`snapshot`、`help`、`quit` 等命令。`ReplicaChooser` 实现读副本轮询和写副本过滤。`QueryPostProcessor`、`MiniSqlResultParser` 和 `QueryTable` 负责解析 MiniSQL 文本输出，并完成聚合、排序和 join 的中心化合并。

`datanode` 模块负责本地持久化和 MiniSQL 执行。`DataNodeServer` 暴露 `/execute`、`/wal`、`/recovery-state`、`/health`、`/admin/refresh-shards`、`/admin/snapshot`，并提供 `executeLocalSql` 供本地 CLI 调用。`DataNodeCli` 是面向运维和调试的 **DataNode Local CLI**：在已启动的 `DataNodeServer` 进程内循环读取 stdin，调用 `executeLocalSql` 执行 SQL；退出 CLI 不会停止 DataNode HTTP 服务。`WalLog` 和 `WalEntry` 实现 JSON Lines WAL。`SnapshotManager`、`SnapshotManifest` 维护 snapshot 文件、已压缩 WAL 序号和各分片追赶进度。
...
flowchart TD
    ...
    StartCoord --> Serve[后台 HTTP API]
    Serve --> StartClient[前台 Coordinator Client]
    StartNodes --> OptionalNodeCli[可选: datanode --cli]
```

## 4.1 交互式 Client 设计

系统提供两类 CLI，职责不同，互不替代。

### Coordinator Client（`CoordinatorCli`）
...
或通过 `./run.sh` 在集群就绪后自动进入前台 client。

设计要点：

- **进程分离**：client 与 Coordinator 是两个 JVM 进程。退出 client（`quit`/`exit`/EOF）不会停止后台 Coordinator、DataNode 和 ZooKeeper。
- **HTTP 代理**：所有集群操作都通过 Coordinator HTTP API 完成。普通 SQL 走 `POST /sql`；`nodes`、`metadata` 走 GET；`rebalance`、`snapshot` 走 POST admin 接口。
- **分片键显式指定**：`shard <key> <sql>` 会把 `shardKey` 填入 `SqlRequest`，用于测试单分片读写路由。
- **适用场景**：日常分布式功能验证、DDL/写入/跨分片查询、扩容 rebalance、集群健康检查。

### DataNode Local CLI（`DataNodeCli`）
...
- **进程内调用**：CLI 与 DataNode 同进程，直接调用 `DataNodeServer.executeLocalSql`，不写入 Coordinator 路由，也不向其他副本复制。
- **只读本地副本状态**：用于检查本节点 MiniSQL 数据、排查 WAL 重放结果、对比副本间数据差异；不能替代分布式写入入口。
- **CLI 会话目录**：`executeLocalSql` 在 `dataDir/cli-session` 维护持久 MiniSQL 工作目录。首次使用或 WAL/snapshot 压缩点变化时，后台静默重放 snapshot SQL 与增量 WAL；之后每条命令只执行当前 SQL，终端只显示当前语句输出，避免重复打印历史 WAL 的执行结果。
- **与 HTTP `/execute` 的区别**：HTTP 路径每次在临时目录中重放 WAL 后执行，保证无状态一致性；CLI 路径为交互体验优化，在会话有效期内复用已重放状态。

```mermaid
flowchart TD
    subgraph coordSide [集群入口]
        CC[Coordinator Client] -->|HTTP| CS[CoordinatorServer]
        CS -->|/execute| DN1[DataNode]
        CS -->|/execute| DN2[DataNode]
    end
    subgraph nodeSide [单节点调试]
        NC[DataNode CLI --cli] -->|executeLocalSql| DNS[DataNodeServer]
        DNS -->|cli-session 目录| Mini[MiniSQL batch]
    end
```
...
4. 调用 `electPrimary`，如果当前 primary 不在在线 `SERVING` 节点中，则在该分片存活副本中选择 **当前集群 primary 数量最少** 的节点作为新 primary（同负载时按 `nodeId` 字典序），并递增 term。
...
- `GET /health`：返回当前节点状态。
- `GET /recovery-state`：返回 snapshot manifest、snapshot SQL 和本地 WAL，供其他副本恢复时拉取。
- `POST /admin/refresh-shards`：刷新自己负责的分片，进入恢复流程并追赶 WAL。
- `POST /admin/snapshot`：生成 snapshot 并压缩 WAL。
- `executeLocalSql(sql)`：供 `DataNodeCli` 在本进程内执行 SQL（见 4.1 节）。
...
- `databases/`：MiniSQL 当前工作数据目录（snapshot 创建时复制源）。
- `cli-session/`：DataNode Local CLI 使用的持久 MiniSQL 工作目录；WAL 或 snapshot 变化时会重建并重放。

### 8.3 DataNode Local CLI 执行路径

`executeLocalSql` 流程：

1. 比较当前 `walLog.lastSequence()` 与 `snapshotManifest.lastCompactedWalSequence` 是否与会话缓存一致。
2. 若不一致，清空并重建 `cli-session`，在目录内重放 `replaySqlBefore(Long.MAX_VALUE)`，不向前台打印重放输出。
3. 在 `cli-session` 目录中仅执行用户当前 SQL，返回本次 stdout。

该路径不追加 WAL，也不更新 ZooKeeper，因此只适合本节点调试读查询；通过 CLI 执行的写入只影响本地 `cli-session` 中的 MiniSQL 状态，不会进入分布式复制链路。
...
写入时 Coordinator 会先确认 primary 是否在线且处于 `SERVING` 状态。如果不是，则在该分片存活副本中按 **最少 primary 负载** 策略选举新 primary，并递增 term。
...
Coordinator 会读取当前 live nodes，并调用 `ZkMetadataStore.rebalance` 重新计算每个 shard 的副本集合。rebalance 分两步：

1. **副本布局**：使用与初始化相同的轮转策略 `liveIds[(shardId + replica) mod liveIds.size()]`，输入为当前在线节点（含 `RECOVERING`）。
2. **Primary 均衡**：对所有分片调用 `assignBalancedPrimaries`，在每个分片的副本集合内，依次为 shard 选择当前 primary 计数最少的节点作为 primary，使各节点承担的 primary 数量尽量均匀；同计数时按 `nodeId` 排序保证确定性。

若副本集合或 primary 发生变化，则递增该 shard 的 `term` 并写回 ZooKeeper。
...
### 12.1 Coordinator Client

Coordinator 作为后台 HTTP 服务持续运行。前台 **Coordinator Client**（`client` 子命令或 `./run.sh`）是面向集群用户的统一交互入口：

| 命令 | 作用 | 对应 HTTP |
|------|------|-----------|
| `<sql>` | 分布式 SQL | `POST /sql` |
| `shard <key> <sql>` | 带分片键的 SQL | `POST /sql`（带 `shardKey`） |
| `nodes` | 在线 DataNode | `GET /nodes` |
| `metadata` | 分片元数据 | `GET /metadata` |
| `health` | 集群健康 | `GET /admin/health` |
| `rebalance` | 重算副本与 primary | `POST /admin/rebalance` |
| `snapshot` | 触发各节点 snapshot | `POST /admin/snapshot` |
| `quit` / `exit` | 退出 client | — |

### 12.2 DataNode Local CLI

**DataNode Local CLI**（`datanode ... --cli`）用于单节点调试：

| 输入 | 作用 |
|------|------|
| `<sql>` | 在本节点 `cli-session` 中执行 SQL，仅显示当前语句输出 |
| `quit` / `exit` | 退出 CLI，DataNode HTTP 服务继续运行 |

典型用途：对比某副本本地数据、验证 WAL 重放后表内容、排查恢复问题。分布式一致性测试仍应使用 Coordinator Client。

### 12.3 HTTP 运维 API

HTTP API 供自动化测试、脚本和 Client 调用。Coordinator 运维接口包括：
...
这些接口既用于自动化测试，也方便手工排查。推荐用法：

- **分布式功能与一致性**：`./run.sh` 进入 Coordinator Client，或单独启动 `client` 子命令。
- **单副本数据排查**：`datanode <config> <nodeId> --cli` 进入 DataNode Local CLI。
- **关闭整套系统**：`./stop-system.sh`。

## 13. MiniSQL 调用设计

DataNode 通过 `MiniSqlCli` 调用 C++ MiniSQL，不直接链接 C++ 代码。`MiniSqlCli` 提供两种执行模式：

**无状态模式**（`execute`，HTTP `/execute` 路径）：

1. 在 `workDir` 下创建临时目录 `minisql-exec-*`。
2. 将 snapshot SQL、增量 WAL 和当前 SQL 写入 batch 文件。
3. 调用 `minisql --batch <file>`，执行完毕后删除临时目录。
4. 返回完整 stdout 给 Coordinator。

**会话模式**（`executeInDirectory`，DataNode Local CLI 路径）：

1. 使用固定目录（`cli-session` 或调用方指定路径）。
2. 首次或 WAL 变化时在该目录内重放历史 SQL（输出丢弃）。
3. 后续命令仅追加当前 SQL 到 batch 并执行，只返回本次输出。

对于有默认数据库的配置，`MiniSqlCli` 会在合适场景下插入 `use <defaultDatabase>`，使 DataNode 不需要每次请求都显式携带数据库上下文。

HTTP 路径保持无状态重放，保证与 WAL 一致；CLI 路径用会话目录减少重复重放和输出噪音。两者仍以子进程 batch 方式调用 MiniSQL，对内核侵入低，但进程启动与 SQL 重放开销仍存在。

## 14. 测试设计

系统提供两类测试方式。

自动化端到端测试脚本是 `scripts/e2e_system_test.py`。它会自动启动临时 ZooKeeper、三个初始 DataNode 和 Coordinator，执行建库建表、多数派写入、聚合、排序、join、单节点故障、第四节点扩容、rebalance、health、snapshot 和最终查询。

手动测试推荐使用仓库根目录的 `run.sh`。该脚本会构建系统，先清理固定端口上的残留进程，再启动 ZooKeeper 和 DataNode，等待 DataNode 进入 `SERVING` 后后台启动 Coordinator，并以前台 `client` 角色进入交互 CLI。用户在 client 中执行 SQL 和运维命令；输入 `quit`、`exit`、Ctrl+C 或遇到 stdin EOF 只会退出 client，后台系统继续运行。需要关闭系统时执行 `./stop-system.sh`。
...
扩容方面，当前 rebalance 使用简单轮转副本布局和 primary 计数均衡，尚未根据数据量、热点、磁盘容量或网络带宽做智能调度，也没有真正的分片数据迁移。

Stage 2 已增加 Coordinator Client、DataNode Local CLI、CLI 会话执行和 rebalance 时 primary 均衡分配。更完整的后续扩展方向见 `docs/项目总结与扩展方向-stage2.md`。
